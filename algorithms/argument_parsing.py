...
print("--- DEBUG: LOADING FILE -> algorithms/argument_parsing.py ---")
def add_algorithms_arguments(
    ...
    print("--- DEBUG: BEFORE calling add_fedclusterinit_arguments ---")

    parser = add_fedclusterinit_arguments(parser)
    print("--- DEBUG: AFTER calling add_fedclusterinit_arguments ---")
...
def add_fedclusterinit_arguments(
        parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    print("--- DEBUG: EXECUTING function add_fedclusterinit_arguments ---")
    ...
    # --- 新增开始 ---
    # 为鲁棒计数机制添加新的命令行参数
    parser.add_argument('--counting-method', default='original', type=str,
                        choices=['original', 'histogram'],
                        help='The robust counting method to use in FedDP-Init Step 2.')
    parser.add_argument('--num-bins', default=128, type=int,
                        help='Number of bins for the histogram counting method.')
    print("--- DEBUG: FINISHED adding --counting-method and --num-bins ---")
