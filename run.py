...
from utils import (set_seed, add_utils_arguments, post_evaluation, make_results_path,
                   kmeans_initialise_sphere_packing) # 删除了 maybe_inject...
...
def main():
    ...
    if temp_args.args_config:
        with open(temp_args.args_config, 'r') as f:
            config = yaml.safe_load(f)
        # 将配置文件中的值设置为解析器的默认值
        parser.set_defaults(**config)

    # 用更新了默认值后的解析器，正式解析所有参数
    ...
    if args.clustering_algorithm != 'None' and args.fedlloyds_num_iterations != 0:
        ...
        if args.clustering_algorithm == 'FederatedLloyds':
            ...
            args.fedlloyds_cohort_size = args.num_train_clients # For simplicity sample_embedded_data all clients per round
