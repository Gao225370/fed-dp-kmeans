...
def _make_worker_fn(topics, save_dir, data_dir, lock):
    ...
    def _extract_users(args):
        ...
        #---------修改后-----------
        # 每个进程各写一个分片文件，避免并发写同一 H5
        pid = os.getpid()
        save_filename = f'{split}_users_pid{pid}.hdf5'
        h5_path = os.path.join(save_dir, save_filename)

        # 写时打开一次，迭代里不要重复打开
        with h5py.File(h5_path, 'a') as h5:
            for user_id, (x, y) in user_data.items():
                grp = h5.require_group(f'/{split}/{user_id}')
                # 已存在就跳过（防止重复创建导致 name already exists）
                if 'embeddings' not in grp:
                    grp.create_dataset('embeddings', data=x, chunks=True)
                if 'labels' not in grp:
                    grp.create_dataset('labels', data=y, chunks=True)
